...
export default function Page() {
  ...
  const load = () => {
    ...
    apiClient
      .get(`/fnb/pos/statement?${params.toString()}`)
      .then((res) => {
    ...
    <ProtectedRoute>
      <div className="flex h-screen bg-gray-200">
        ...
        <main className="ml-64 flex-1 overflow-y-auto p-6">
          <h1 className="text-3xl font-bold text-gray-900">POS Customer Statement</h1>
          <p className="mt-1 text-gray-600">Sales, payments at sale, and due receives for one customer.</p>
          ...
          <div className="mt-4 overflow-auto rounded-xl border bg-white">
            {loading ? (
              ...
              <p className="p-8 text-center text-slate-500">Select a customer and run the statement.</p>
            ) : (
                  ...
                  {rows.map((row, idx) => (
                    <tr key={idx} className="border-t">
                      {cellKeys.map((k) => (
                        <td
                          key={k}
                          className={`px-3 py-2 ${typeof row[k] === 'number' ? 'text-right' : ''}`}
                        >
                          {typeof row[k] === 'number'
